...
- Stealth Fetch/XHR responses now enter Obscura's bounded network-event and response-body buffers, allowing Scorch to observe their privacy and freshness directives. CORS preflights use the same Chrome-like wreq/BoringSSL transport as the scripted request rather than exposing a reqwest/rustls fingerprint, and cached public scripts report their resident age instead of restarting origin freshness. Obscura now exposes monotonic author-active-content provenance, so Scorch conservatively declines to retain extracted output from any page containing scripts or inline event handlers.

### Changed

- Scrapes now retain eligible extracted results in a bounded in-memory cache for up to five minutes. The foreground request extracts and returns only its requested formats; an eight-job, 16 MiB background queue then prepares the remaining formats without retaining raw HTML in the result cache. Entries are capped at 8 MiB each, 256 entries, and a 64 MiB weighted process budget; redirects, final-URL changes, cookies, author scripts or inline event handlers, private or `no-store` responses, unsafe `Vary`, and stale origin freshness bypass retention. `maxAgeMs: 0` forces a refresh and `storeInCache: false` bypasses reads and writes. In the final conservative policy, second requests to three script-free public pages—Example Domain, httpbin HTML, and CERN's first website—completed in 1.1–1.4 ms. On the README's mixed four-page workload, where three pages contain active content and deliberately rerender, twelve warm requests had a 566 ms median and an eight-request trial had a 2.237 s median (3.58 req/s). Background-warmed formats for the eligible static page completed in 0.7–1.2 ms.
