...
- Stealth Fetch/XHR responses now enter Obscura's bounded network-event and response-body buffers, allowing Scorch to honor their privacy and freshness directives before retaining extracted output. CORS preflights use the same Chrome-like wreq/BoringSSL transport as the scripted request rather than exposing a reqwest/rustls fingerprint, and cached public scripts report their resident age instead of restarting origin freshness.
